# To-Do: Going Public

Checklist of improvements before/after making the repository public.

## Before Making Public

- [x] Add `LICENSE` file (MIT)
- [x] Add root `README.md` with features, tech stack, build instructions, and doc links
- [ ] Add a screenshot or demo GIF of the dashboard to `README.md`

## Strengthening Portfolio Value

- [ ] Add tests — at least a few critical-path unit tests
  - Backend: xUnit (workflow engine, plugin loader, API auth)
  - Frontend: Vitest + React Testing Library
- [ ] Add GitHub Topics tags to the repository (`dotnet`, `react`, `typescript`, `desktop-app`, `plugin-system`, `wpf`, `webview2`)
- [ ] Pin the repository on your GitHub profile

## Nice to Have

- [ ] Add a Mermaid architecture diagram to `README.md`
- [ ] Add `CONTRIBUTING.md`
